...
export const createBootcamp = asyncHandler(async (req, res, next) => {
  // Add user to req.body for the mongoose model
  ...
  if (publishedBootcamp && req.user.role !== 'admin') {
    return next(new ErrorResponse(`User with ID that ends with '${req.user.id.slice(-6)}' has already published a bootcamp`, 400));
  }
...
export const updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(new ErrorResponse(`User with ID that ends with '${req.user.id.slice(-6)}' is not authorized to update this bootcamp`, 401));
  }

  bootcamp = await Bootcamp.findByIdAndUpdate(req.params.id, req.body, {
    // Returns the updated bootcamp
    new: true,
    runValidators: true,
  });
...
export const deleteBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // Make sure user is bootcamp owner
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
    return next(new ErrorResponse(`User with ID that ends with '${req.user.id.slice(-6)}' is not authorized to delete this bootcamp`, 401));
  }
